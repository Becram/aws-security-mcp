...
CROSS_ACCOUNT_ROLE_NAME = "aws-security"
...
def create_progress_bar(current: int, total: int, width: Optional[int] = None, fill_char: str = "█", empty_char: str = "░") -> str:
    ...
    if width is None:
        # Cap at 50 characters for very large account counts
        width = min(total, 50)
...
class CredentialSession:
    ...
    def get_client(self, service_name: str):
        ...
        if self.is_expired():
            raise ValueError(
                f"Session for account {self.account_id} has expired")
...
async def discover_organization_accounts() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Discovered {len(active_accounts)} active accounts in organization")
...
async def assume_cross_account_role(account_id: str, account_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug(
            f"Successfully assumed role in account {account_id} ({account_name})")
        ...
        logger.debug(
            f"Failed to assume role in account {account_id} ({account_name}): {str(e)}")
...
async def setup_cross_account_sessions() -> Dict[str, Any]:
    ...
    async def update_progress(success: bool = False, failed: bool = False):
        ...
        async with progress_lock:
            ...
            if not config.server.startup_quiet:
                ...
                print(
                    f"\rAssuming roles: {progress_bar} {processed_accounts}/{total_accounts} accounts ({status_text})", end="", flush=True)

    # Show initial progress
    if not config.server.startup_quiet:
        progress_bar = create_progress_bar(0, total_accounts)
        print(
            f"\rAssuming roles: {progress_bar} 0/{total_accounts} accounts", end="", flush=True)
    ...
    async def process_account(account: Dict[str, Any]) -> Dict[str, Any]:
        async with semaphore:
            ...
            current_account = get_client(
                'sts').get_caller_identity().get('Account')
            if account_id == current_account:
                logger.debug(
                    f"Skipping current account {account_id} ({account_name})")
    ...
    if not config.server.startup_quiet:
        ...
        if successful_sessions == 0 and failed_sessions == 0:
            logger.info(
                "Cross-account setup complete: No accounts required role assumption")
        elif failed_sessions == 0:
            logger.info(
                f"Cross-account setup complete: {successful_sessions}/{total_accounts} accounts accessible")
        else:
            logger.info(
                f"Cross-account setup complete: {successful_sessions} successful, {failed_sessions} failed ({total_accounts} total)")
...
async def refresh_expired_sessions() -> Dict[str, Any]:
    ...
    failed_count = 0
    refresh_details = []
    expired_sessions = [session_key for session_key,
                        session in _account_sessions.items() if session.is_expired()]
    ...
    if not config.server.startup_quiet:
        ...
        print(
            f"\rRefreshing sessions: {progress_bar} 0/{total_sessions} sessions", end="", flush=True)
    ...
    for session_key, session in list(_account_sessions.items()):
        if session.is_expired():
            ...
            if assume_result["success"]:
                ...
                logger.debug(
                    f"Failed to refresh session {session_key}: {assume_result.get('error')}")
            ...
            if not config.server.startup_quiet:
                ...
                print(
                    f"\rRefreshing sessions: {progress_bar} {processed_sessions}/{total_sessions} sessions ({status_text})", end="", flush=True)
    ...
    if not config.server.startup_quiet:
        ...
        if failed_count == 0:
            logger.info(
                f"Session refresh complete: {refreshed_count} sessions refreshed successfully")
        else:
            logger.info(
                f"Session refresh complete: {refreshed_count} successful, {failed_count} failed")
